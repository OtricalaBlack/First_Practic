...
int main()
{
	...
	else
	{
		...
		while (!file_1)
		{
			...
			// Если пользователь захочет прекратить попытки ввода корректного имени файла
			printf("Продолжить? (Введите \"Yes/yes\", если хотите продолжить): ");
			scanf("%s", &next);

			printf("\n");

			// Если пользователь введет не "Yes\yes", то выход из программы
